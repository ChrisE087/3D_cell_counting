                ...
                if ext == '.nrrd':
                    ...
                    for subdir3 in subdirs3:
                        ...
                        if spheroid_name in subdir3 and not "24h" in subdir1 and not "48h" in subdir1:
                            #if 'C2-untreated_4' in spheroid or 'C2-untreated_4.1' in spheroid or 'C2-untreated_4.2' in spheroid:
                            ...
                            patches_X = impro.gen_patches(data=X, patch_slices=size_z, patch_rows=size_y, 
                                                    ...
                                                    stride_cols=size_x, input_dim_order='XYZ', padding='SAME') #ZYX
                            ...
                            patches_y = impro.undo_data_normalization(patches_y, max_val, min_val)
                            ...
                            for a in range (patches_X.shape[0]):
                                for b in range (patches_X.shape[1]):
                                    for c in range (patches_X.shape[2]):
                                        ...
                                        # Generate a sample
                                        sample = np.zeros(shape=(2, patch_x.shape[0], patch_x.shape[1], patch_x.shape[2]), dtype=np.float64)
                                        sample[0] = patch_x
                                        sample[1] = patch_y
                                        ...
                                        sample_name = "%s_%s-%04d.nnrd" % (time_range, spheroid_name, p)
