...
with tf.Session() as sess:
    t = tf.extract_volume_patches(vol_in_exp, ksizes=[1, 6, 3, 3, 1], 
                                  strides=[1, 6, 3, 3, 1], 
                                  padding='VALID').eval()
    print(t)
...
y2x1 = np.reshape(y2x1, (6,3,3))
y2x2 = np.reshape(y2x2, (6,3,3))

# Build the input volume
a = np.concatenate((y1x1, y1x2), axis=2)
b = np.concatenate((y2x1, y2x2), axis=2)
