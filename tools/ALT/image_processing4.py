...
def standardize_data(input_data):
    ...
    Parameters:
    input_data (Numpy Array): ND Numpy Array to standardizate.
    
    Returns:
    normalized_data (Numpy Array): Standardizated data.
    mean (float): Mean of the original data.
    sigma (float): Standard deviation of the original data.
    """
    mean = np.mean(input_data)
    sigma = np.std(input_data)
    standardizated_data = (input_data-mean)/sigma
    return standardizated_data, mean, sigma
